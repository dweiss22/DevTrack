import type { SupabaseClient } from "@supabase/supabase-js";

export type VideoDevelopmentYear = {
  year: number;
  projectCount: number;
  averageMinutes: number | null;
  totalMinutes: number | null;
};

export type VideoCategoryAverage = {
  name: string;
  averageMinutes: number;
  totalMinutes: number;
  percentage: number;
};

export type VideoCategoryPeriod = {
  year: number | null;
  qualifyingProjectCount: number;
  totalMinutes: number;
  entryCount: number;
  categories: VideoCategoryAverage[];
};

export type VideoDashboardAnalytics = {
  timeDataSynchronized: boolean;
  developmentTimeByYear: VideoDevelopmentYear[];
  categoryTime: {
    denominatorDefinition: string;
    allTime: VideoCategoryPeriod;
    years: VideoCategoryPeriod[];
  };
};

export type VideoDashboardAnalyticsResult =
  | { data: VideoDashboardAnalytics; error: null }
  | { data: null; error: string };

const EMPTY_PERIOD: VideoCategoryPeriod = {
  year: null,
  qualifyingProjectCount: 0,
  totalMinutes: 0,
  entryCount: 0,
  categories: [],
};

export const EMPTY_VIDEO_DASHBOARD_ANALYTICS: VideoDashboardAnalytics = {
  timeDataSynchronized: false,
  developmentTimeByYear: [],
  categoryTime: {
    denominatorDefinition: "Distinct Single Video projects assigned to the selected videographer on which they logged time, grouped by calendar year.",
    allTime: EMPTY_PERIOD,
    years: [],
  },
};

export async function loadVideoDashboardAnalytics(
  supabase: SupabaseClient,
  targetWrikeUserId: string,
): Promise<VideoDashboardAnalyticsResult> {
  try {
    const { data, error } = await supabase.rpc("reporting_video_dashboard_analytics", {
      target_wrike_user_id: targetWrikeUserId,
    });
    if (error) {
      console.error("video_dashboard_analytics_failed", { code: error.code });
      return {
        data: null,
        error: error.code === "PGRST202" || error.code === "42883"
          ? "Video Dashboard analytics require the latest database migration."
          : "Video Dashboard analytics could not be loaded.",
      };
    }
    return { data: normalizeVideoDashboardAnalytics(data), error: null };
  } catch {
    return { data: null, error: "Video Dashboard analytics could not be loaded." };
  }
}

export function normalizeVideoDashboardAnalytics(value: unknown): VideoDashboardAnalytics {
  if (!value || typeof value !== "object") return EMPTY_VIDEO_DASHBOARD_ANALYTICS;
  const candidate = value as Partial<VideoDashboardAnalytics>;
  const categoryTime = candidate.categoryTime;
  return {
    timeDataSynchronized: Boolean(candidate.timeDataSynchronized),
    developmentTimeByYear: Array.isArray(candidate.developmentTimeByYear)
      ? candidate.developmentTimeByYear.map((row) => ({
        year: Number(row.year),
        projectCount: Number(row.projectCount),
        averageMinutes: row.averageMinutes == null ? null : Number(row.averageMinutes),
        totalMinutes: row.totalMinutes == null ? null : Number(row.totalMinutes),
      }))
      : [],
    categoryTime: {
      denominatorDefinition: categoryTime?.denominatorDefinition
        ?? EMPTY_VIDEO_DASHBOARD_ANALYTICS.categoryTime.denominatorDefinition,
      allTime: normalizeCategoryPeriod(categoryTime?.allTime, null),
      years: Array.isArray(categoryTime?.years)
        ? categoryTime.years.map((period) => normalizeCategoryPeriod(period, Number(period.year)))
        : [],
    },
  };
}

export function categoryPeriodForYear(
  analytics: VideoDashboardAnalytics,
  year: "all" | number,
) {
  if (year === "all") return analytics.categoryTime.allTime;
  return analytics.categoryTime.years.find((period) => period.year === year) ?? {
    ...EMPTY_PERIOD,
    year,
  };
}

function normalizeCategoryPeriod(value: unknown, fallbackYear: number | null): VideoCategoryPeriod {
  if (!value || typeof value !== "object") return { ...EMPTY_PERIOD, year: fallbackYear };
  const period = value as Partial<VideoCategoryPeriod>;
  return {
    year: period.year == null ? fallbackYear : Number(period.year),
    qualifyingProjectCount: Number(period.qualifyingProjectCount ?? 0),
    totalMinutes: Number(period.totalMinutes ?? 0),
    entryCount: Number(period.entryCount ?? 0),
    categories: Array.isArray(period.categories)
      ? period.categories.map((category) => ({
        name: category.name || "Uncategorized",
        averageMinutes: Number(category.averageMinutes ?? 0),
        totalMinutes: Number(category.totalMinutes ?? 0),
        percentage: Number(category.percentage ?? 0),
      })).filter((category) => category.totalMinutes > 0)
      : [],
  };
}
