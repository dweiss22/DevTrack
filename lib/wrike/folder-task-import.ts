...
import { designerAssignedPersonIdentityObservations, syncTaskPersonIdentities, taskPersonIdentityObservations } from "@/lib/wrike/person-identity";
...
async function runFolderTaskImport(db: ReturnType<typeof createAdminClient>, organizationId: string, runId: string, startedAt: Date, tracker: ImportTracker) {
  ...
  const identitySync = await syncTaskPersonIdentities(db, organizationId, client, [
    ...taskPersonIdentityObservations(tasks, enrichedByTaskId),
    ...designerAssignedPersonIdentityObservations(tasks, enrichedByTaskId)
  ]);
