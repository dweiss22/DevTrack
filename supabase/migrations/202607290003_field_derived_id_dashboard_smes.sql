-- Resolve ID Dashboard SMEs from the same durable, field-derived identities
-- used by Project Details and the SME Dashboard. ID ownership remains governed
-- by the authoritative ID Assigned resolver.

drop function if exists public.reporting_id_dashboard_rows(uuid);

create function public.reporting_id_dashboard_rows(
...
begin
  ...
  select task.id,task.title,coalesce(status.title,task.status),
    ...
    sme_identity.display_name,coalesce(sme_member.email,wrike_sme.email),
...
comment on function public.reporting_id_dashboard_rows(uuid) is
  'ID-assigned projects with one row per durable SME custom-field identity. Survey matches prefer sme_identity_id and retain reviewed_wrike_user_id only as a historical fallback.';
