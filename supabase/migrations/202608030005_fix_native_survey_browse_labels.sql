-- Native survey context is task-centric. Field-derived SMEs may not have a
-- Wrike user id, so resolve their trusted reviewedSme context before falling
-- back to legacy subject/Wrike fields in the Admin Submissions table.
  ...
  with combined as (
    select survey.id,survey.survey_type,survey.status,survey.is_locked,
      ...
      coalesce(survey.context_snapshot->>'taskTitle',
        survey.context_snapshot->>'projectTitle','Unavailable') project_title,
      coalesce(survey.context_snapshot#>>'{reviewedSme,name}',
        survey.context_snapshot->>'reviewedSmeName',
        survey.context_snapshot#>>'{subject,name}',reviewed.display_name,
        'Unavailable') sme_name,
      ...
      coalesce(survey.context_snapshot->>'taskTitle',
        survey.context_snapshot->>'projectTitle') historical_course_name,
