-- Restrict Development Percentile benchmarks to genuinely comparable,
-- completed Online Learning projects with reliable source evidence.
...
begin
  ...
  if matched is not null then
    result := matched[1]::numeric * 60 + matched[2]::numeric;
  else
    matched := regexp_match(
      normalized,
      '^(\d+(?:\.\d+)?)\s*(?:hours?|hrs?|h)\s*(?:and\s*)?(\d+(?:\.\d+)?)\s*(?:minutes?|mins?|m)$'
    );
    if matched is not null then
      result := matched[1]::numeric * 60 + matched[2]::numeric;
    else
      ...
      if matched is not null then
        result := matched[1]::numeric * 60;
      else
        matched := regexp_match(normalized, '^(\d+(?:\.\d+)?)\s*(?:minutes?|mins?|m)$');
        if matched is not null then
          result := matched[1]::numeric;
        else
          -- Unitless integers and decimals are deliberately ambiguous.
          return null;
        end if;
      end if;
    end if;
  ...
  -- Cohort keys are exact integer minutes; never round an ambiguous fraction.
  if result<=0 or result<>trunc(result) or result>2147483647 then return null; end if;
...
as $$
  with parsed as (
    ...
    select count(*) as value_count,
      count(*) filter(where minutes is null) as invalid_count,
  ...
  select case
    when value_count>0 and invalid_count=0 and distinct_count=1 then minutes
    else null
  end
  from evidence;
$$;

create or replace function public.wrike_course_style_value(source_value text)
returns text
language sql
immutable
set search_path=public
as $$
  select case lower(regexp_replace(trim(source_value), '\s+', ' ', 'g'))
...
create or replace function public.wrike_course_style(source_values text[])
returns text
language sql
immutable
set search_path=public
as $$
  with parsed as (
    select public.wrike_course_style_value(value) as style
    from unnest(coalesce(source_values,'{}'::text[])) value
  ), evidence as (
    select count(*) as value_count,
      count(*) filter(where style is null) as invalid_count,
      count(distinct style) filter(where style is not null) as distinct_count,
      min(style) filter(where style is not null) as style
    from parsed
  )
  select case
    when value_count>0 and invalid_count=0 and distinct_count=1 then style
    else null
  end
  from evidence;
$$;

comment on function public.wrike_course_length_value_minutes(text) is
  'Conservatively parses unit-bearing or HH:MM course lengths to exact positive integer minutes; unitless values and fractional minutes are rejected.';
comment on function public.wrike_course_length_minutes(text[]) is
  'Returns one exact duration only when every populated source value is valid and all values normalize to the same integer minutes.';
comment on function public.wrike_course_style_value(text) is
  'Canonical Course Style vocabulary confirmed in production: Full Length and Single Video, matched case- and whitespace-insensitively without aliases.';
comment on function public.wrike_course_style(text[]) is
  'Returns one canonical Course Style only when every source value is recognized and all values normalize to the same style.';

-- This is the sole eligibility path used by the user-facing benchmark RPCs and
-- the administrator audit. It is internal and cannot be called through
-- PostgREST by ordinary authenticated users.
drop function if exists public.reporting_project_percentile_evidence(uuid);
create function public.reporting_project_percentile_evidence(target_organization_id uuid)
returns table (
  task_id uuid,
...
as $$
  with organization_tasks as materialized (
    ...
    select task.id as task_id,
      ...
      case when status_ref.workflow_id='IEACHQK7K4BHMLHM' then 'Online Learning' end
        as workflow_name,
      ...
      (
        ...
        and status_ref.workflow_id='IEACHQK7K4BHMLHM'
        and (task.workflow_id is null or task.workflow_id='IEACHQK7K4BHMLHM')
        and status_ref.dashboard_classification='completed'
  ...
  select evidence.task_id,
    ...
    case
      ...
      when evidence.status_id is null or evidence.status_is_unresolved
        or evidence.dashboard_classification is null then 'completion_status_unresolved'
      when evidence.status_workflow_id<>'IEACHQK7K4BHMLHM'
        or (
          evidence.task_workflow_id is not null
          and evidence.task_workflow_id<>'IEACHQK7K4BHMLHM'
        ) then 'wrong_workflow'
      when evidence.dashboard_classification<>'completed' then 'project_not_completed'
...
revoke all on function public.reporting_project_percentile_evidence(uuid) from public;
revoke all on function public.reporting_project_percentile_evidence(uuid) from authenticated;
grant execute on function public.reporting_project_percentile_evidence(uuid) to service_role;
...
as $$
  ...
  ), cohort_stats as materialized (
    select eligible.length_minutes,
      eligible.normalized_course_style,
      round(avg(eligible.total_logged_minutes)::numeric,2) as average_minutes,
      round((
        percentile_cont(0.5) within group(order by eligible.total_logged_minutes)
      )::numeric,2) as median_minutes,
      count(*)::bigint as cohort_size
    from eligible
    group by eligible.length_minutes,eligible.normalized_course_style
  ), ranked as materialized (
    select eligible.task_id,
      (rank() over(
        partition by eligible.length_minutes,eligible.normalized_course_style
        order by eligible.total_logged_minutes
      )-1)::bigint as lower_count,
      count(*) over(
        partition by eligible.length_minutes,eligible.normalized_course_style,
          eligible.total_logged_minutes
      )::bigint as tie_count
    from eligible
  )
  select target.task_id,
    ...
    stats.average_minutes as cohort_average_minutes,
    stats.median_minutes as cohort_median_minutes,
    coalesce(stats.cohort_size,0)::bigint as cohort_size,
    rank_counts.lower_count,
    rank_counts.tie_count,
    case
      when target.eligibility_reason is not null then target.eligibility_reason
      when coalesce(stats.cohort_size,0)<5 then 'not_enough_completed_comparable_courses'
      else null
    end as unavailable_reason
  from target_evidence target
  left join cohort_stats stats
    on stats.length_minutes=target.length_minutes
    and stats.normalized_course_style=target.normalized_course_style
  left join ranked rank_counts on rank_counts.task_id=target.task_id;
$$;
...
comment on function public.reporting_project_length_percentiles(uuid[]) is
  'Empirical logged-time midrank inputs for up to 200 requested tasks. Cohorts contain only completed, reliable Online Learning projects with the exact normalized duration and Course Style; the target is included once.';
comment on function public.reporting_project_length_percentile(uuid) is
  'Single-task compatibility wrapper for the completed exact-duration-and-style Development Percentile benchmark.';
  ...
  with evidence as materialized (
    ...
    select candidate.*,
      case
        ...
        when candidate.length_minutes<>(select length_minutes from target) then 'wrong_duration'
    ...
    'includedMembers',coalesce((
      select jsonb_agg(
        jsonb_build_object(
          ...
          'inclusionReason','eligible_exact_duration_style_completed'
        )
...
comment on function public.admin_reporting_project_percentile_audit(uuid) is
  'Administrator-only read-only evidence for one Development Percentile target, including exact cohort members and reason-coded excluded organization candidates.';
