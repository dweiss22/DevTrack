...
function TimeCategoryTooltip({ active, payload, total }: { active?: boolean; payload?: { payload: DevelopmentTimeCategoryMetric }[]; total: number }) { const row = payload?.[0]?.payload; return active && row ? <div className="chart-tooltip"><strong>{row.name}</strong><span>{hours(row.minutes)} total hours</span><span>{statusPercentage(row.minutes, total).toFixed(1)}% of reported hours</span><span>{row.projectCount} contributing projects</span></div> : null; }
function EmptyChart({ message }: { message: string }) { return <p className="chart-empty">{message}</p>; }
function StatusDataTable({ rows, valueHeader, extraHeader, labelHeader = "Status" }: { rows: string[][]; valueHeader: string; extraHeader?: string; labelHeader?: string }) { return <details className="chart-data"><summary>View accessible data</summary><table><thead><tr><th>{labelHeader}</th><th>{valueHeader}</th><th>Percentage</th>{extraHeader&&<th>{extraHeader}</th>}</tr></thead><tbody>{rows.map((row)=><tr key={row[0]}>{row.map((value,index)=><td key={index}>{value}</td>)}</tr>)}</tbody></table></details>; }
