...
export function AdminPanel({ connection, folderRuns, folders, unresolvedReferences, verticalDiagnostics, verticalDiagnosticsError, repairRuns, importConflicts, importConflictCount, importConflictError, historicalBatches, historicalRows, historicalIssues, historicalMappings, historicalTemplates, historicalResolutionOptions, historicalResolutionAudit }: Props) {
  ...
  async function repairVerticals() {
    ...
    try {
      ...
      setComplete(true);
      reloadWithMessage(`Vertical repair complete — ${payload.repaired} repaired, ${payload.unchanged} unchanged, ${payload.unresolved} unresolved, ${payload.conflicting} conflicting, and ${payload.failed} failed.`);
  ...
  return <div className="admin-stack">
    ...
    <AdminDisclosure title="Import & repair" description="Synchronize Wrike data or repair stored Vertical values." defaultOpen id="data-import">
    ...
    <AdminDisclosure title="Import conflicts" description="Review projects where normalized Wrike source fields currently disagree." count={importConflictError ? "Unavailable" : `${importConflictCount} current`} defaultOpen={importConflictCount > 0}>
      <ImportConflictReview conflicts={importConflicts} totalCount={importConflictCount} loadError={importConflictError} />
    </AdminDisclosure>
    ...
    <AdminDisclosure title="History" description="Review recent imports and repairs, newest first." count={`${folderRuns.length + repairRuns.length} recent runs`} id="data-history"><div className="admin-history-toolbar"><p className="muted">Most recent runs are shown first.</p><button className="secondary" onClick={clearHistory} disabled={clearingHistory || folderRuns.length + repairRuns.length === 0}>{clearingHistory ? "Clearing history…" : "Clear history"}</button></div><div className="admin-history-section"><h3>Combined import history</h3>{folderRuns.length ? <table><thead><tr><th>Started</th><th>Status</th><th>Records</th><th>Requests</th><th>Diagnostics</th></tr></thead><tbody>{folderRuns.map((run) => <tr key={run.id}><td>{new Date(run.created_at).toLocaleString()}<br />{run.duration_ms != null ? `${run.duration_ms} ms` : "Running"}</td><td>{run.status}<br />{run.reference_warning_count ?? 0} warning(s)<br />{run.custom_field_conflict_count ?? 0} field conflict(s)<br />{run.unresolved_reference_count ?? 0} unresolved reference(s)</td><td>{run.task_count} tasks<br />{run.unique_timelog_count} timelogs</td><td>{run.task_request_count} task<br />{run.timelog_request_count} timelog</td><td>{run.error_summary ? <>{run.error_summary}<FailureDetails failures={run.folder_failures} /></> : <><span>Descendants: {run.timelog_descendant_strategy}</span><br /><span>Logical custom fields: {run.custom_field_normalization_diagnostics?.logicalFieldCount ?? 0}; values: {run.custom_field_normalization_diagnostics?.normalizedTaskValueCount ?? 0}</span><br /><ReferenceEvidence diagnostics={run.reference_data_diagnostics} /><br /><MetadataEvidence diagnostics={run.metadata_diagnostics} /></>}</td></tr>)}</tbody></table> : <p className="empty">No combined folder import has run yet.</p>}</div><div className="admin-history-section"><h3>Vertical repair history</h3>{repairRuns.length ? <div className="admin-table-wrap"><table><thead><tr><th>Started</th><th>Status</th><th>Examined</th><th>Repaired</th><th>Unchanged</th><th>Unresolved</th><th>Conflicts</th><th>Failed</th></tr></thead><tbody>{repairRuns.map((run) => <tr key={run.id}><td>{new Date(run.started_at).toLocaleString()}</td><td>{run.status}{run.error_summary ? <><br /><span className="error">{run.error_summary}</span></> : null}</td><td>{run.examined_count}</td><td>{run.repaired_count}</td><td>{run.unchanged_count}</td><td>{run.unresolved_count}</td><td>{run.conflicting_count}</td><td>{run.failed_count}</td></tr>)}</tbody></table></div> : <p className="empty">No Vertical repair has run yet.</p>}</div></AdminDisclosure>
  </div>;
}

function VerticalRepairDiagnostics({ diagnostics }: { diagnostics: VerticalDiagnostics }) {
  const repair = diagnostics.repair;
  const projects = repair?.projects ?? [];
  return <div>
    <details>
      <summary>Current organization-scoped diagnostic summary</summary>
      <pre>{JSON.stringify({ quality: diagnostics.quality, repairCounts: repair?.counts, generatedAt: repair?.generatedAt }, null, 2)}</pre>
    </details>
    {projects.length ? <details>
      <summary>{projects.length} unresolved project diagnostic(s)</summary>
      <div className="admin-table-wrap"><table><thead><tr><th>Project</th><th>Wrike Vertical source</th><th>Parsed / normalized</th><th>Reason</th><th>Timestamps</th></tr></thead><tbody>
        {projects.map((project) => <tr key={project.projectId}>
          <td><strong>{project.projectTitle}</strong><br /><code>{project.wrikeTaskId}</code></td>
          <td>{project.verticalFieldTitles.length ? project.verticalFieldTitles.map((title, index) => <span key={`${project.verticalFieldIds[index]}-${title}`}><code>{project.verticalFieldIds[index]}</code> — {title}<br /></span>) : "No configured Vertical field"}<details><summary>Raw stored value</summary><pre>{JSON.stringify(project.rawStoredValue, null, 2)}</pre></details></td>
          <td>{project.parsedDisplayValues.join(", ") || "No parsed values"}<br /><strong>{project.normalizedResult.join(", ") || "Unresolved"}</strong></td>
          <td>{project.reason.replaceAll("_", " ")}<br /><span className="muted">{project.verticalState}</span></td>
          <td>Sync: {project.lastSynchronizedAt ? new Date(project.lastSynchronizedAt).toLocaleString() : "Not verified"}<br />Repair: {project.lastRepairedAt ? new Date(project.lastRepairedAt).toLocaleString() : "Not repaired"}</td>
        </tr>)}
      </tbody></table></div>
    </details> : <p className="empty">All synchronized Vertical values are resolved.</p>}
  </div>;
}
