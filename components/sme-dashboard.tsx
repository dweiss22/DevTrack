...
export function SmeDashboard({ identities, selected, rows, canSelect, canViewProjects, canLaunchDebrief, mappingRequired }: {
      ...
      : !selected
        ...
        : <>
          <p className="card dashboard-identity-note">Showing assignments for <strong>{selected.display_name}</strong>
            {selected.email ? <> ({selected.email})</> : null}. {selected.mapping_status === "unmapped" ? "This person does not yet have a DevTrack account." : "Identity verified and mapped."}</p>
          <div className="metric-grid sme-metrics">
            ...
            <article className="card metric-card"><span>Logged time</span><strong>{hours(rows.reduce((sum, row) => sum + Number(row.actual_minutes || 0), 0))}</strong></article>
          </div>
          {rows.length ? <div className="dashboard-table-wrap"><table className="dashboard-project-table"><thead><tr>
            <th>Course</th><th>Status</th><th>Reporting year</th><th>Original due</th><th>Due / completed</th>
            <th>Project / folder</th><th>Last synchronized</th><th>Debrief</th>
            ...
            return <tr key={row.task_id}>
              <td data-label="Course">{canViewProjects ? <Link href={`/projects/${row.task_id}?returnTo=${encodeURIComponent(returnTo)}`}>{row.title}</Link> : row.title}</td>
              ...
              <td data-label="Original due">{date(row.original_due_date)}</td>
