...
export function UnresolvedReferenceLabel({ id, type, className = "", label }: { id: string; type: WrikeReferenceType; className?: string; label?: string }) {
  const explanation = UNRESOLVED_REFERENCE_MESSAGES[type];
  return <span className={`unresolved-reference ${className}`.trim()} tabIndex={0} aria-label={label ? `${label}. Wrike ID ${id}. ${explanation}` : `${id}. ${explanation}`}>
    {label ?? <code>{id}</code>}<CircleHelp size={14} aria-hidden="true" />
    <span className="unresolved-tooltip" role="tooltip">{explanation} Wrike ID: {id}</span>
  </span>;
}
