...
export type ManagedMember = { id: string; name: string; email: string; role: Role; createdAt: string };
export type ManagedInvitation = { id: string; email: string; role: Role; status: "pending" | "failed"; invitedAt: string; lastSentAt: string | null; lastError: string | null };

export function UserManagementPanel({ members, invitations }: { members: ManagedMember[]; invitations: ManagedInvitation[] }) {
  ...
  return <div className="admin-stack">
    ...
    <section className="card" aria-labelledby="add-user-title">
      ...
      <form className="user-invite-form" onSubmit={invite}>
        ...
        <label>Application role<select name="role" defaultValue="member"><option value="member">Member</option><option value="admin">Administrator</option></select></label>
    ...
    <section className="user-members-section" aria-labelledby="pending-invitations-title">
      ...
      {invitations.length ? <div className="admin-table-wrap"><table><thead><tr><th>Email</th><th>Status</th><th>Role</th><th>Sent</th><th>Actions</th></tr></thead><tbody>{invitations.map((invitation) => {
        ...
        return <tr key={invitation.id}><td>{invitation.email}</td><td>{invitation.status === "failed" ? "Email failed" : "Invitation pending"}{invitation.lastError ? <><br /><span className="error">{invitation.lastError}</span></> : null}</td><td><select aria-label={`Role for ${invitation.email}`} value={invitation.role} disabled={Boolean(submitting)} onChange={(event) => request(endpoint, "PATCH", { action: "change_role", role: event.target.value }, `Role updated for ${invitation.email}.`)}><option value="member">Member</option><option value="admin">Administrator</option></select></td><td>{invitation.lastSentAt ? new Date(invitation.lastSentAt).toLocaleString() : "Not sent"}</td><td><div className="table-actions"><button className="secondary" disabled={Boolean(submitting)} onClick={() => request(endpoint, "PATCH", { action: "resend" }, `Invitation resent to ${invitation.email}.`)}>Resend</button><button className="secondary danger" disabled={Boolean(submitting)} onClick={() => { if (confirm(`Cancel the invitation for ${invitation.email}?`)) void request(endpoint, "PATCH", { action: "cancel" }, `Invitation canceled for ${invitation.email}.`); }}>Cancel</button></div></td></tr>;
    ...
    <section className="user-members-section" aria-labelledby="organization-members-title">
      ...
      {members.length ? <div className="admin-table-wrap"><table><thead><tr><th>User</th><th>Email</th><th>Role</th><th>Added</th></tr></thead><tbody>{members.map((member) => <tr key={member.id}><td>{member.name}</td><td>{member.email}</td><td><select aria-label={`Role for ${member.name}`} value={member.role} disabled={Boolean(submitting)} onChange={(event) => request(`/api/admin/users/${member.id}`, "PATCH", { role: event.target.value }, `Role updated for ${member.email}.`)}><option value="member">Member</option><option value="admin">Administrator</option></select></td><td>{new Date(member.createdAt).toLocaleDateString()}</td></tr>)}</tbody></table></div> : <p className="card empty">No application users are assigned to this organization.</p>}
    </section>
  </div>;
}
