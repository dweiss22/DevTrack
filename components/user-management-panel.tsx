...
export function UserManagementPanel({ members, invitations, identities }: { members: ManagedMember[]; invitations: ManagedInvitation[]; identities: IdentityOption[] }) {
  ...
  return <div className="admin-stack">
    ...
    <section className="user-members-section" aria-labelledby="organization-members-title">
      ...
      {members.length ? <div className="admin-table-wrap"><table><thead><tr><th>User</th><th>Email</th><th>Role</th><th>Wrike identity</th><th>Added</th></tr></thead><tbody>{members.map((member) => {
        const locked = member.role === "super_admin";
        const canMap = member.role === "sme" || member.role === "id";
        return <tr key={member.id}><td>{member.name}</td><td>{member.email}</td><td>{locked ? <><strong>{roleLabel(member.role)}</strong><br /><span className="muted">Fixed account</span></> : <select aria-label={`Role for ${member.name}`} value={member.role} disabled={Boolean(submitting)} onChange={(event) => request(`/api/admin/users/${member.id}`, "PATCH", { role: event.target.value }, `Role updated for ${member.email}.`)}><option value="id">ID</option><option value="sme">SME</option><option value="admin">Admin</option></select>}</td><td>{canMap ? <select aria-label={`Wrike identity for ${member.name}`} value={member.wrikeUserId ?? ""} disabled={Boolean(submitting)} onChange={(event) => request(`/api/admin/users/${member.id}/wrike-identity`, "PATCH", { wrikeUserId: event.target.value || null }, `Wrike identity updated for ${member.email}.`)}><option value="">Not mapped</option>{identities.map((identity) => <option key={identity.id} value={identity.id}>{identity.name}{identity.email ? ` (${identity.email})` : ""}</option>)}</select> : "Not applicable"}</td><td>{new Date(member.createdAt).toLocaleDateString()}</td></tr>;
