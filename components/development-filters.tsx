  ...
  return <section aria-label="Development filters">
    <form className="card report-filters development-filters" method="get">
      <div className="filter-fields">
        <label>Reporting year<select name="reportingSelection" defaultValue={filters.reportingYearMode === "missing" ? "missing" : `year:${filters.reportingYear}`}><option value="" disabled>Select year</option>{years.years.map((year) => <option key={year.year} value={`year:${year.year}`}>{year.label} ({year.projects})</option>)}<option value="missing">Missing/Unresolved ({years.missingProjects})</option></select></label>
