...
| `GET /contacts?name={encodedName}&types=["Person"]&active=true` or `&emails=[...]` | During the combined background import for deduplicated readable first-and-last names found in Contacts custom fields or already waiting in `wrike_person_identities`. Email lookups are batched in groups of at most 100; remaining names use bounded concurrency. | Verifies a task-provided identity only for one exact email match or one exact normalized-name match. Ambiguous, missing, and failed matches remain displayable but unverified and are retained for administrator review. Successful contact IDs and mappings are cached locally. | [`lib/wrike/person-identity.ts`](../lib/wrike/person-identity.ts), called by [`lib/wrike/folder-task-import.ts`](../lib/wrike/folder-task-import.ts) | [Query contacts](https://developers.wrike.com/reference/getcontactsempty) |
...
`GET /users/{userId}` requires `amReadOnlyUser`, so connections authorized only with `wsReadOnly` must reconnect. It remains limited to ID-based account-management details such as profile role and active state. Basic task-name verification uses `GET /contacts` with `wsReadOnly` instead. Wrike's returned name is authoritative. If it differs from the configured expected name, DevTrack retains Wrike's value and records both values as a warning. A configured fallback row is created only when no row exists and never overwrites previously synchronized data.

Readable task-provided person names are displayable immediately and are not labeled unresolved merely because they have not been verified. Verification trims names, collapses repeated whitespace, compares case-insensitively, prefers one exact email match, and otherwise accepts only one exact normalized-name match. Multiple candidates are never chosen automatically. The reusable background batch also loads due unverified identities from the database, not only identities observed in the current task response. Ambiguous matches retry after 24 hours, request failures after one hour, and names not found after seven days. Zero-match, ambiguous, and failed attempts, attempt counts, diagnostics, and next retry times remain available on the Administration page, while successful mappings and canonical contact fields are cached in `wrike_person_identities` and `wrike_users` for later pages and imports.
...
- Users display a readable synchronized, historical, configured, or task-provided name without an unresolved marker. Displayability and verification are stored separately. Only a raw ID receives the concise **Name unavailable** treatment. Missing, unresolved, or more-than-24-hour-old encountered IDs are deduplicated and refreshed with concurrency four; readable Contacts-field names use the cached contact-verification workflow described above.
...
An unresolved ID is never presented as if it were a meaningful name. For people, the interface displays the raw ID with a concise **Name unavailable** marker and accessible explanation, without the old help tooltip. Other unresolved reference types keep their warning/help treatment. Dense custom-field columns put that explanation on the heading. Ask DevTrack describes an unresolved value as an unresolved Wrike reference rather than returning the bare ID as a name.
