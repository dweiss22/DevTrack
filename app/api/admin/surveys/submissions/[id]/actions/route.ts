...
const schema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("unlock"), reason: z.string().trim().min(1).max(2000), reviserId: z.null().optional() }),
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const call = body.action === "unlock"
    ? supabase.rpc("survey_unlock", { target_submission_id: id, unlock_reason_text: body.reason, assigned_reviser_id: null })
    : body.action === "relock"
  ...
  return error
    ? NextResponse.json({ error: error.message || "The survey action could not be completed." }, { status: error.code === "42501" ? 404 : 400 })
