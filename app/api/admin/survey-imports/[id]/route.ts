...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [batch, attempts, mappings, rows, issues, integrations, audit] = await Promise.all([
  ...
  const error = [attempts, mappings, rows, issues, integrations, audit].find((result) => result.error)?.error;
