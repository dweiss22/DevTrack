...
export async function GET() {
  ...
  try {
    ...
    const { data: lastRun } = await createAdminClient().from("wrike_folder_task_import_runs").select("status,created_at,task_count,error_summary").eq("organization_id", profile.organization_id).order("created_at", { ascending: false }).limit(1).maybeSingle();
