...
export default async function TeamPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const filters = parseReportingFilters(await searchParams); const { supabase, profile } = await requirePageCapability("view_standard_pages"); const [summary, options] = await Promise.all([loadTimeSummary(supabase, filters, "person"), loadReportingOptions(supabase, profile.organization_id)]);
