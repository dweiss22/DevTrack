...
export default async function Dashboard({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [analyticsResult, statuses, customFields, lastRunResult] = await Promise.all([
  ...
  return <AppShell isAdmin={profile.role === "admin"} lastSynced={lastRunResult.data?.created_at}>
    ...
    </section> : analytics && <DashboardContent analytics={analytics} filters={filters} statuses={statuses} customFields={customFields} />}
  </AppShell>;
}

function DashboardContent({ analytics, filters, statuses, customFields }: { analytics: DashboardAnalytics; filters: ReturnType<typeof parseReportingFilters>; statuses: Awaited<ReturnType<typeof loadStatusOptions>>; customFields: Awaited<ReturnType<typeof loadCustomFieldOptions>> }) {
