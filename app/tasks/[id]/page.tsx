...
export default async function TaskDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: task }, { data: users }, { data: categories }, { data: statuses }] = await Promise.all([
    ...
    supabase.from("wrike_users").select("wrike_id,display_name,email,avatar_url,synced_at").eq("organization_id", profile.organization_id),
    supabase.from("wrike_timelog_categories").select("wrike_id,title").eq("organization_id", profile.organization_id),
    supabase.from("wrike_workflow_statuses").select("wrike_id,title").eq("organization_id", profile.organization_id)
  ]);
  if (!task) notFound();
  ...
  const statusName = resolveTaskStatus(row.custom_status_id, row.status, statuses ?? []).name;

  return <AppShell>
    <header className="page-header"><div><p className="eyebrow">TASK DETAIL</p><h1>{row.title}</h1><p>{statusName} · Due {row.due_date ?? "not set"}</p></div>{row.permalink && <a className="button" href={row.permalink} target="_blank" rel="noreferrer">Open in Wrike</a>}</header>
    <div className="admin-grid">
      <section className="card"><h2>Reporting details</h2><p><strong>Assignees:</strong> {assignees.map((item) => item.fullName).join(", ") || "Unassigned"}</p><p><strong>Planned effort:</strong> {row.planned_minutes == null ? "Not available" : `${hours(row.planned_minutes)} hours`}</p><p><strong>Allocated effort:</strong> {row.allocated_minutes == null ? "Not available" : `${hours(row.allocated_minutes)} hours`}</p><p><strong>Completion:</strong> {row.completed_at ? new Date(row.completed_at).toLocaleString() : "Not completed"}</p><p>{row.description || "No task description supplied by Wrike."}</p></section>
    ...
    {profile.role === "admin" && <section className="card"><h2>Administrator: original Wrike metadata</h2><p><strong>Responsible IDs:</strong> {(row.responsible_wrike_ids ?? []).join(", ") || "None"}<br /><strong>Custom status ID:</strong> {row.custom_status_id ?? "None"}</p><h3>Resolved raw custom fields</h3><pre>{JSON.stringify(customFieldsRaw, null, 2)}</pre><h3>Original task response</h3><pre>{JSON.stringify(row.raw_data, null, 2)}</pre></section>}
    <section className="card"><h2>Visible time entries</h2>{row.wrike_time_entries.length ? <table><thead><tr><th>Date</th><th>Person</th><th>Category</th><th>Hours</th><th>Comment</th></tr></thead><tbody>{row.wrike_time_entries.map((entry) => {
      const person = entry.user_wrike_id ? resolveResponsibleUsers([entry.user_wrike_id], users ?? [])[0]?.fullName : entry.wrike_users?.display_name;
      const category = resolveTimelogCategory(entry.category, categories ?? []);
      return <tr key={entry.id}><td>{entry.entry_date}</td><td>{person ?? entry.user_wrike_id ?? "Unknown"}</td><td>{category?.name ?? "—"}</td><td>{hours(entry.minutes)}</td><td>{entry.comment ?? "—"}</td></tr>;
    })}</tbody></table> : <p className="empty">No visible recorded time for this task.</p>}</section>
  </AppShell>;
}
