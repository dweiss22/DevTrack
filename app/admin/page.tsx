...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ connected?: string; error?: string }> }) {
  ...
  const [{ data: connection }, { data: scopes }, { data: runs }, { data: groups }, { data: appUsers }, { data: wrikeUsers }, { data: fields }, { data: enabledFields }, { data: settings }, { data: audit }] = await Promise.all([
    ...
    supabase.from("organizations").select("timezone,reporting_access_enforced,ask_enabled").eq("id", profile.organization_id).single(),
  ...
  return <AppShell><header className="page-header"><div><p className="eyebrow">ADMINISTRATION</p><h1>Wrike data connection</h1><p>Maintain synchronization, reporting access, custom fields, and question auditing.</p></div></header>{params.connected && <p className="notice">Wrike was connected successfully.</p>}{params.error && <p className="notice error">{params.error}</p>}<AdminPanel connection={connection} scopes={scopes ?? []} runs={(runs ?? []) as never} groups={(groups ?? []) as never} appUsers={(appUsers ?? []).map((item) => ({ id: item.id, name: item.display_name ?? item.id }))} wrikeUsers={(wrikeUsers ?? []).map((item) => ({ id: item.id, name: item.display_name }))} customFields={(fields ?? []).map((field) => ({ ...field, enabled: enabled.has(field.id) }))} settings={settings ?? { timezone: "America/Chicago", reporting_access_enforced: false, ask_enabled: false }} audit={audit ?? []} /></AppShell>;
}
