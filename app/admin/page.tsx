...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ connected?: string; error?: string }> }) {
  ...
  return <AppShell><header className="page-header"><div><p className="eyebrow">ADMINISTRATION</p><h1>Wrike task and metadata import</h1><p>One button validates the folder tree, LCT custom fields, and the configured task endpoints before replacing reporting data.</p></div></header>{params.connected && <p className="notice">Wrike connected successfully. The import has not run until you select the button below.</p>}{params.error && <p className="notice error">{params.error}</p>}<AdminPanel connection={connection} folderRuns={(folderRuns ?? []) as never} folderIds={TASK_IMPORT_FOLDER_IDS} /></AppShell>;
}
