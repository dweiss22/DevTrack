...
export default function ProjectsLoading() {
  return <AppShell isAdmin={false}>
    <div className="projects-loading" aria-busy="true" aria-live="polite">
      ...
      <div className="projects-table-wrap" aria-hidden="true"><table className="projects-table projects-loading-table"><thead><tr><th>Project name</th><th>Status</th><th>Vertical</th><th>Designer Assigned</th><th>Course Style</th><th>Folders</th><th>Development percentile</th></tr></thead><tbody>{Array.from({ length: 8 }, (_, row) => <tr key={row}>{Array.from({ length: 7 }, (_, cell) => <td key={cell}><span className="loading-pulse" /></td>)}</tr>)}</tbody></table></div>
    </div>
  </AppShell>;
}
